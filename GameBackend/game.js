...
function getPieceFromChar(char) {
	switch (char) {
		case "♜":
			return new Rook("R", false);

		case "♞":
			return new Knight("N", false);

		case "♝":
			return new Bishop("B", false);

		case "♚":
			return new King("K", false);

		case "♛":
			return new Queen("Q", false);

		case "♟":
			return new Pawn("p", false);

		case "♖":
			return new Rook("R", true);

		case "♘":
			return new Knight("N", true);

		case "♗":
			return new Bishop("B", true);

		case "♔":
			return new King("K", true);

		case "♕":
			return new Queen("Q", true);

		case "♙":
			return new Pawn("p", true);
	...
	constructor(boardposition) {
		for (let rank = 1; rank <= 8; rank++) {
			...
			for (let file of ["a", "b", "c", "d", "e", "f", "g", "h"]) {
				...
				t.piece = getPieceFromChar(boardposition[7 - t.row][t.col]);
...
class Pawn extends Piece {
	...
	get oneforward() {
		if (this.color == true) {
			return board.getTile(this.tile.row + 1, this.tile.col);
		} else {
			return board.getTile(this.tile.row - 1, this.tile.col);
		}
	}
	...
	get twoforward() {
		if (this.color == true) {
			return board.getTile(this.tile.row + 2, this.tile.col);
		} else {
			return board.getTile(this.tile.row - 2, this.tile.col);
		}
	}
	...
	get LeftTop() {
		if (this.color == true) {
			return board.getTile(this.tile.row + 1, this.tile.col - 1);
		} else {
			return board.getTile(this.tile.row - 1, this.tile.col + 1);
		}
	}
...
class Knight extends Piece {
   ...
   */
	...
	get r1() {
		if (this.color == true) {
			return board.getTile(this.tile.row + 1, this.tile.col - 2);
		} else {
			return board.getTile(this.tile.row - 1, this.tile.col + 2);
		}
	}
	...
	get r2() {
		if (this.color == true) {
			return board.getTile(this.tile.row + 2, this.tile.col - 1);
		} else {
			return board.getTile(this.tile.row - 2, this.tile.col + 1);
		}
	}
	...
	get up() {
		...
		if (this.color == true) {
			for (let row = this.tile.row + 1; row <= 7; row++) {
				tile = board.getTile(row, this.tile.col);
	...
	get left() {
		...
		if (this.color == true) {
			for (let col = this.tile.col - 1; col >= 0; col--) {
				tile = board.getTile(this.tile.row, col);
...
class Game {
	...
	move(tile1, tile2) {
		...
		if (tile2.hasPiece && tile2.piece.color == game.turn) return false;

		let r = board.move(tile1.piece, tile2);
...
const createNewGame = () => {
	...
	game.newGame(board, new Player("amit", true), new Player("pradeep", false));
};

module.exports = { game, createNewGame };
